...
const Feed = () => {
    ...
    if (searchLoading || (!searchValue.trim().length && !globalResults.length && !localResults.length)) {
        return <FeedSkeleton skeletonsCount={3} animate={searchLoading} />;
    }
    ...
    return !filteredLocalResults.length && !filteredGlobalResults.length ? (
        <>
